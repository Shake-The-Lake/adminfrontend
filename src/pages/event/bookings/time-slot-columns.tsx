...
export const timeSlotColumns = (
	...
	[
...

		{
			...
			cell({ row }) {
				return `${row.original.availableViewerSeats ?? 0} / ${row.original.seatsViewer} ${t('booking.free')}`;
			},
		},
		{
			...
			cell({ row }) {
				return `${row.original.availableRiderSeats ?? 0} / ${row.original.seatsRider} ${t('booking.free')}`;
			},
		},
	];
